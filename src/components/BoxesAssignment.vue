...
export default {
  ...
  methods: {
    startPainting: function (event) {
      this.draw_mode = true;
      const box_selected = getBoxSelected(this.boxlist, event, this.canvas);
      if ([0, 2].includes(box_selected.type)) {
    ...
    draw: function (event) {
      ...
      //     2 inside rectangle
      //     3 min point
      this.current_box = getBoxSelected(this.boxlist, event, this.canvas);
